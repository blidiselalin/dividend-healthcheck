...
Overview · Income · Risk · Research · Import
"""
...
def _tour_completed_count() -> int:
    return sum(
        1
        for key in (_CC_TOUR_ADJUSTED, _CC_TOUR_INCOME, _CC_TOUR_RISK, _CC_TOUR_RESEARCH)
        if st.session_state.get(key)
    )
...
def resolve_next_demo_step() -> DemoStep:
    ...
    if not st.session_state.get(_CC_TOUR_ADJUSTED):
        return DemoStep(
    ...
    if not st.session_state.get(_CC_TOUR_INCOME):
        return DemoStep(
            title="Compare estimated and received concepts",
            description=(
    ...
    if not st.session_state.get(_CC_TOUR_RISK):
        return DemoStep(
            title="Inspect a risk signal",
            description="Review attention labels from the sample portfolio's safety alerts.",
            action_label="Open Risk",
            target_page=DemoPage.RISK,
        )
    if not st.session_state.get(_CC_TOUR_RESEARCH):
        return DemoStep(
            title="Review score evidence",
            description="Open Research to inspect yield channels and library evidence.",
            action_label="Open Research",
            target_page=DemoPage.RESEARCH,
        )
    return DemoStep(
        title="Create your portfolio",
        description="Save your try list privately and import a real IBKR statement.",
...
def _render_demo_nav(active: DemoPage) -> None:
    pages = [
        ...
        (DemoPage.INCOME, "Income"),
        (DemoPage.RISK, "Risk"),
        (DemoPage.RESEARCH, "Research"),
        (DemoPage.IMPORT, "Import"),
    ]
...
def _render_next_demo_step() -> None:
    step = resolve_next_demo_step()
    done = _tour_completed_count()
    label = "Guided demo complete" if done >= 4 else f"{done} of 4 complete"
    st.caption(f"Next demo step · {label}")
    st.markdown(f"**{step.title}**")
    st.caption(step.description)
    if step.target_page is None:
        if st.button(step.action_label, key="cc_demo_step_auth", type="primary"):
            request_auth_panel(source_section="demo_step")
            st.rerun()
    elif st.button(step.action_label, key=f"cc_demo_step_{step.target_page.value}"):
...
def _render_overview(dashboard: GuestDashboard) -> None:
    render_section_header(
        ...
        "Adjust holdings — estimated income updates on the next rerun.",
    )
    _render_next_demo_step()
    ...
    if holdings:
        ...
        for holding in holdings:
            ...
            if abs(new_shares - holding.shares) > 1e-9:
                add_guest_holding(st.session_state, symbol=holding.symbol, shares=new_shares)
                track_guidance_event(
    ...
    if dashboard.holdings:
        render_ticker_chips([(h.symbol, f"{h.shares:.0f} sh") for h in dashboard.holdings])
    ...
    if st.button("Reset to KO, JNJ, O", key="cc_demo_reset"):
    ...
    st.caption(f"Holdings: {len(dashboard.holdings)}")
...
def _render_income_chart(dashboard: GuestDashboard) -> None:
    if not dashboard.monthly_forecast:
        render_empty_state(
            "No forecast yet",
            "Add sample holdings to see an illustrative monthly forecast.",
        )
        return
    ...
    try:
        ...
        st.bar_chart(dict(dashboard.monthly_forecast), height=280)


def _render_income(dashboard: GuestDashboard) -> None:
    _mark_tour(_CC_TOUR_INCOME, step_id="income")
    render_section_header(
        "Estimated income vs illustrative receipts",
        "The public sample has no broker cash history.",
    )
    ...
    st.caption("Label: Estimated · Projected · Illustrative forecast")
    ...
    if dashboard.next_payouts:
        ...
        for payout in dashboard.next_payouts[:5]:
            pay = payout.pay_date.strftime("%d %b %Y") if payout.pay_date else "TBD"
            st.markdown(
                f"**{payout.symbol}** · ${payout.amount_usd:,.2f} · pay ~{pay} · {payout.status}"
            )
    ...
    # Static educational example — not derived from guest holdings as "received cash".
    example_gross = 548.0
    example_tax = 82.0
    example_net = 466.0
    render_metric_grid(
        [
            (term_label("gross_dividend"), f"${example_gross:,.2f}", "Example only"),
            (term_label("withholding_tax"), f"${example_tax:,.2f}", "Example only"),
            (term_label("net_dividend"), f"${example_net:,.2f}", "Example only"),
        ]
    )
    st.info(
        "Illustrative received-income example — not connected to a broker account. "
        "Authenticated portfolios use imported broker cash transactions for received totals."
    )
...
def _render_risk(dashboard: GuestDashboard) -> None:
    ...
    render_section_header(
        "Portfolio attention",
        "From sample safety alerts — review aids, not buy or sell recommendations.",
    )
    if not dashboard.safety_alerts:
        render_empty_state(
            "No attention items",
            "Adjust the sample list or wait for library metrics.",
        )
        st.caption("Not enough data from the shared library for Watch / Risky signals.")
        return

    for alert in dashboard.safety_alerts:
        severity = _severity_label(alert.severity)
        with st.expander(f"{severity} · {alert.symbol}", expanded=False):
            st.markdown(f"**{alert.company or alert.symbol}**")
            st.write(alert.message)
            st.caption("Source: shared market library · portfolio row metrics")
            st.caption("Next step: compare payout, yield, and growth on Research.")
...
def _render_research(dashboard: GuestDashboard) -> None:
    ...
    render_section_header(
        f"Research · {spotlight}",
        "Yield channel and library evidence — not a personalized recommendation.",
    )
    ...
    st.caption(
        "Educational research only. Existing score and yield-channel components are reused — "
        "no separate composite score is calculated for the public demo."
    )


def _render_import() -> None:
    render_section_header(
        "Import preview (simulation)",
        "Real broker files are accepted only after you create an account.",
    )
    st.markdown(
        """
**What DividendScope can import**
...
"""
    ...
    st.warning("File upload is disabled on the public demo for security.")
    render_metric_grid(
        [
    ...
    st.caption("Example statement range and currencies shown after authentication.")
    with st.expander("Example reconciliation", expanded=False):
        for label, status in (
...
def render_public_demo(*, route: PublicRoute, dashboard: GuestDashboard) -> None:
    ...
    st.caption("Interactive demo · session-only sample portfolio · no account required")
    ...
    render_page_divider()
    if st.button("Create your portfolio", key="cc_demo_footer_auth", use_container_width=True):
        request_auth_panel(source_section="demo_footer")
        st.rerun()
