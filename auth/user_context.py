...
@contextmanager
def bind_background_user_id(user_id: str | None):
    """
...
def clear_portfolio_session_state() -> None:
    ...
    keys = [
        ...
        "portfolio_fast_loaded",
        "_portfolio_stale_cache_loaded",
        "_portfolio_load_attempted",
        "_month_paid_cache",
        "_month_paid_cache_day",
        "_month_paid_cache_fp",
        "_stale_prices_pending",
        "_deferred_price_refresh_scheduled",
        _SESSION_USER_KEY,
        _SESSION_EMAIL_KEY,
    ]
    for key in keys:
...
def ensure_user_session() -> AppUser | None:
    """
    ...
    if is_test_user(user):
        ...
        if (user.is_admin or is_admin_email(user.email)) and restore_owner_portfolio(
            ...
            clear_portfolio_session_state()
        # Legacy portfolio is not auto-copied on first login. Per-user portfolio data is stored in
