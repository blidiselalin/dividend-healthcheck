...
def test_run_price_refresh_once_updates_stats(monkeypatch):
...
def test_start_price_refresh_scheduler_is_idempotent(monkeypatch):
...
def test_scheduler_disabled_under_pytest_sqlite(monkeypatch):
