...
def test_net_received_through_applies_withholding() -> None:
    ...
    assert net_received_through(100.0, year=2026) == 90.0
...
def test_current_month_paid_prefers_synced_receipts_over_compute(
    ...
    monkeypatch.setattr(
        "services.portfolio_month_dividends.create_portfolio_context",
        lambda: type(
            "Ctx",
            (),
            {"portfolio": type("Store", (), {"list_open_holdings": lambda self: [holding]})()},
        )(),
    )
    monkeypatch.setattr(
...
def test_current_month_paid_returns_zero_snapshot_for_rows(
    ...
    monkeypatch.setattr(
        ...
        lambda: type(
            "Ctx",
            (),
            {"portfolio": type("Store", (), {"list_open_holdings": lambda self: []})()},
        )(),
    )
    monkeypatch.setattr(
        "services.portfolio_month_dividends.gross_paid_in_calendar_month",
