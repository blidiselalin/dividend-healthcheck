...
def test_summarize_with_rows_uses_estimated_annual_net() -> None:
    ...
    records = [
        ...
        _record(year=2026, month=1, net=90.0, gross=100.0, tax=10.0),
        _record(year=2026, month=2, net=180.0, gross=200.0, tax=10.0),
    ]

    # 360 annual gross -> 10% withholding => 324 net annual => 27 monthly avg
    ...
    assert summary.total_net_usd == 360.0
    assert summary.ytd_year == 2026
    assert summary.ytd_net_usd == 270.0
    assert summary.best_year == 2026
    assert summary.best_year_net == 270.0
    assert summary.avg_monthly_net == 27.0
...
def test_pivot_and_timeline_dataframes() -> None:
    ...
    assert pivot[pivot["Month"] == "Jan"].iloc[0]["2026"] == 90.0
    ...
    assert list(timeline["cumulative_net_usd"]) == [90.0, 180.0, 360.0]
