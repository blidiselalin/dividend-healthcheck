...
def test_compute_holdings_summary_totals():
    rows = [
        _row(
            ticker="INTU",
            shares=100.0,
...
def test_day_change_missing_when_no_previous_close():
...
def test_partial_previous_close_excludes_missing_symbols():
    rows = [
        _row(ticker="A", previous_close=100.0, current_price=101.0, shares=10.0, current_value=1010.0),
        _row(ticker="B", previous_close=None, current_price=50.0, shares=10.0, current_value=500.0),
    ]
    summary = compute_holdings_summary(rows)
