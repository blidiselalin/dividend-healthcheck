...
def test_build_rows_without_live_prices_has_no_previous_close_without_history(portfolio_store):
...
def test_build_rows_without_live_prices_uses_history_previous_close(portfolio_store):
    ...
    with patch.object(service, "_load_documents", return_value={"KO": document}), patch(
        "services.portfolio_details_service.load_portfolio_statistics_stock",
        return_value=_stats(),
    ), patch.object(service, "_get_price_snapshot", return_value=_empty_snapshot()), patch.object(
        service, "_get_market_extras", return_value=(None, None, None)
...
def test_build_rows_with_live_prices_falls_back_to_history_previous_close(portfolio_store):
    from datetime import date
