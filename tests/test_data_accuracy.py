...
def store() -> Any:
    """Provide the vector database store as a fixture."""
...
def test_database_availability(store: Any) -> None:
    """Test that the vector database is available and has data."""
    assert store.count() > 0


def test_dividend_kings_count(store: Any) -> Any:
    """Test that we have a reasonable number of Dividend Kings."""

    all_docs = store.get_all_documents()
    if len(all_docs) < 400:
        pytest.skip(
            f"Database only has {len(all_docs)} docs (expected ~500), skipping kings validation."
        )
...
def test_stock_data_accuracy(store: Any) -> Any:  # noqa: C901
    """
    ...
    for symbol, expected in expected_stocks.items():
        ...
        if errors:
            ...
            print(f"✓ {symbol}: streak={streak}yrs, yield={yld}, PE={pe}, payout={payout}")
            passed += 1
...
def main() -> int:
    ...
    try:
        ...
        store = VectorStore()
        test_database_availability(store)
    ...
    try:
        test_dividend_kings_count(store)
    ...
    if all_passed:
        ...
        print("  ✗ SOME TESTS FAILED - Review failures")
        print("=" * 60)
        return 1
