...
def fmt_range(start: Optional[date], end: Optional[date], count: int) -> str:
    if count <= 0:
...
def price_history_bounds(doc: Any) -> Tuple[Optional[date], Optional[date], int]:
...
def dividend_history_bounds(doc: Any) -> Tuple[Optional[date], Optional[date], int]:
    history = getattr(doc, "dividend_history", None) or []
    if not history:
        return None, None, 0
    dates = sorted(
        record.ex_date for record in history if getattr(record, "ex_date", None)
    )
    if not dates:
        return None, None, 0
    return dates[0], dates[-1], len(dates)
...
def yield_channel_bounds(channel: Any) -> Tuple[Optional[date], Optional[date], int]:
...
def build_evidence_rows(
    ...
    *,
    ...
    if yield_channel_data:
        ...
        rows.append(
            ("Yield channel analysis window", f"Last {years} years · {interval}")
        )
        ...
        if y10 is not None and y90 is not None:
            zone_detail = (
                f"Weiss-style percentile bands ({y10:.2f}%–{y90:.2f}% historical yield)"
            )
        else:
    ...
    rows.append(
        (
            ...
            "Stored history intervals above + latest reload for prices, payout, streak, and yield zones",
        )
    )
    return rows
