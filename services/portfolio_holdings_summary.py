...
class HoldingsSummary:
    ...
    unrealized_gl_pct: Optional[float]
...
def compute_holdings_summary(rows: List["PortfolioDetailRow"]) -> HoldingsSummary:
    ...
    unrealized_pct = (
        (unrealized / total_acquisition * 100) if total_acquisition > 0 else None
    )
    ...
    day_change_pct = (
        (day_change / prior_value * 100)
        if has_day_change and prior_value > 0
        else None
    )
