...
try:
    ...
    DIVIDEND_KINGS = []  # type: ignore[misc]
    DIVIDEND_ARISTOCRATS = []  # type: ignore[misc]
...
class StockQuoteFetcher(BaseFetcher):
    ...
    def __init__(self, output_dir: str = "data/downloads/stockquote") -> None:
        """
        Initialize fetcher.
    ...
    def _symbols_to_stock_list(self, symbols: list[str], category: str) -> list[dict[str, Any]]:
        """
    ...
    def fetch_stock_details(self, symbol: str) -> dict[str, Any] | None:
        """
    ...
    def fetch_dividend_history(self, symbol: str, years: int = 75) -> list[dict[str, Any]]:
        """
    ...
    def save_dividend_history_to_csv(
        self, history: list[dict[str, Any]], symbol: str | None = None
    ) -> str:
    ...
    def download_all(  # noqa: C901
        ...
        if symbols is None:
            all_symbols = {s["symbol"] for s in kings + aristocrats}
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", "-o", default="data/downloads/stockquote", help="Output directory"
    )
    parser.add_argument("--symbols", "-s", nargs="+", help="Specific symbols to download")
    parser.add_argument("--no-history", action="store_true", help="Skip dividend history download")
