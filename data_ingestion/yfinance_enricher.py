...
from .models import StockDocument, DividendRecord, PriceHistory, DataSource
...
class YFinanceEnricher:
            ...
            for date_idx, amount in dividends.items():
                try:
                    ...
                    if ex_date not in existing_dates and amount > 0:
                        doc.dividend_history.append(DividendRecord(
                            ex_date=ex_date,
                            payment_date=None,
                            amount=float(amount),
                        ))
    ...
    def _enrich_price_data(
        ...
        try:
            ...
            if len(hist) > 0:
                current_price = hist["Adj Close"].iloc[-1] if "Adj Close" in hist.columns else hist["Close"].iloc[-1]
                
                # 1-year return
                if len(hist) >= 252:
                    price_1y_ago = hist["Adj Close"].iloc[-252] if "Adj Close" in hist.columns else hist["Close"].iloc[-252]
                    if price_1y_ago > 0:
                        doc.price_return_1y = ((current_price / price_1y_ago) - 1) * 100
                
                # 5-year return
                if len(hist) >= 1260:
                    price_5y_ago = hist["Adj Close"].iloc[0] if "Adj Close" in hist.columns else hist["Close"].iloc[0]
                    if price_5y_ago > 0:
                ...
                if doc.dividend_history and doc.price_return_1y is not None:
                    ...
                    yearly_divs = sum(
                        d.amount for d in doc.dividend_history
                        if d.ex_date >= one_year_ago
                    )
