...
from .downloaders import BaseDownloader, NasdaqDownloader, StockQuoteDownloader
from .models import SearchResult, StockDocument
...
class DataIngestionPipeline:
    ...
    def __init__(
        ...
        Args:
            data_dir: Base directory for downloaded data.
                Defaults to ~/.dividendscope/data/downloads.
    ...
    def run(  # noqa: C901
        ...
        for source_name in sources:
            ...
            for doc in downloader.process_directory():
                ...
                self._stats[f"{source_name}_parsed"] = (
                    self._stats.get(f"{source_name}_parsed", 0) + 1
                )
        ...
        for _symbol, docs in all_documents.items():
            merged = self._merge_documents(docs, merge_strategy)
            if merged:
                merged_documents.append(merged)
                self._stats["documents_merged"] = self._stats.get("documents_merged", 0) + 1
    ...
    def enrich_existing(
        ...
        if symbols:
            documents = [
                d for d in [self.vector_store.get_by_symbol(s) for s in symbols] if d is not None
            ]
        else:
            # Get all documents from vector store
            all_docs: list[StockDocument] = []
    ...
    def add_symbols_from_yfinance(
        self,
        symbols: list[str],
        progress_callback: Callable[[str, int, int], None] | None = None,
    ) -> dict[str, Any]:
        """
        Create new StockDocument entries directly from yfinance for symbols
        not already in any CSV source.
        ...
        Returns:
            Statistics dict with 'added', 'errors', 'total_documents'.
        """
        if not ENRICHER_AVAILABLE:
            logger.error("YFinanceEnricher not available")
            return {"error": "enricher_not_available"}
        ...
        enricher = YFinanceEnricher(request_delay=0.3)
        stats: dict[str, Any] = {}
        added: list[StockDocument] = []
        total = len(symbols)

        for i, symbol in enumerate(symbols):
            try:
                # Start with a minimal stub document
                stub = StockDocument(
                    symbol=symbol.upper(),
                    name=symbol.upper(),
                )
                enriched = enricher.enrich_document(stub)
                added.append(enriched)
                stats["added"] = stats.get("added", 0) + 1
                if progress_callback:
                    progress_callback(f"Fetched {symbol}", i + 1, total)
            except Exception as e:
                logger.error(f"Error fetching {symbol} from yfinance: {e}")
                stats["errors"] = stats.get("errors", 0) + 1

        if added:
            self.vector_store.add_documents(added)
    ...
    def _merge_two_documents(  # noqa: C901
        self,
...
def create_sample_data(data_dir: str | None = None) -> None:
    """
    ...
    with open(sq_dir / "sample_fundamentals.csv", "w") as f:
        ...
        f.write(
            ...
            "Household Products,350000000000,24.1,2.45,62.3\n"
        )
