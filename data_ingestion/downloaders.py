...
class BaseDownloader(ABC):
    ...
    def _calculate_dividend_streak(self, dividends: list[DividendRecord]) -> int:  # noqa: C901
        """
        Calculate consecutive years of dividend INCREASES.

        Uses the median regular dividend for each year to be robust
        against special dividends and minor date shifts.
        Counts backwards from the most recent COMPLETE year, incrementing
        for each consecutive year where the median dividend >= prior year.
        ...
        annual_divs: dict[int, list[float]] = {}
        for d in dividends:
            year = d.ex_date.year
            if year not in annual_divs:
                annual_divs[year] = []
            annual_divs[year].append(d.amount)

        if len(annual_divs) < 2:
            return 0

        # Calculate median dividend per year
        annual_regular: dict[int, float] = {}
        for year, divs in annual_divs.items():
            sorted_divs = sorted(divs)
            median_div = sorted_divs[len(sorted_divs) // 2]
            annual_regular[year] = median_div

        # Sort years ascending
        years = sorted(annual_regular.keys())
        ...
        for i in range(len(years) - 1, 0, -1):
            ...
            # Ensure years are strictly consecutive
            if current_yr - prior_yr != 1:
                break

            current_div = annual_regular[current_yr]
            prior_div = annual_regular[prior_yr]
...
class StockQuoteDownloader(BaseDownloader):
    ...
    def _parse_dividend_history(self, filepath: Path) -> list[StockDocument]:
            ...
            for row in reader:
                ...
                try:
                    ex_date = self._parse_date(
                        row.get("Ex-Date", row.get("ex_date", row.get("ExDate", "")))
                    )
                    payment_date = self._parse_date(
                        row.get("Payment-Date", row.get("payment_date", row.get("PaymentDate", "")))
                    )
                    amount = self._parse_float(
                        row.get("Amount", row.get("amount", row.get("Dividend", "0")))
                    )
        ...
        for doc in documents.values():
            if doc.dividend_history:
                ...
                doc.dividend_streak_years = self._calculate_dividend_streak(doc.dividend_history)
            ...
            for row in reader:
                ...
                doc = StockDocument(
                    ...
                    dividend_yield=self._parse_float(
                        row.get("DivYield", row.get("Yield", row.get("dividend_yield")))
                    ),
                    annual_dividend=self._parse_float(
                        row.get("AnnualDividend", row.get("annual_dividend"))
                    ),
                    payout_ratio=self._parse_payout_ratio(
                        row.get("PayoutRatio", row.get("payout_ratio"))
                    ),
                    current_price=self._parse_float(
                        row.get("Price", row.get("price", row.get("LastPrice")))
                    ),
    ...
    def _parse_dividend_streaks(self, filepath: Path) -> list[StockDocument]:
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                streak = self._parse_int(
                    row.get(
                        "ConsecutiveYears",
                        row.get("Years", row.get("Streak", row.get("streak", "0"))),
                    )
                )
    ...
    def _parse_payout_ratio(self, value: Any) -> float | None:
...
class NasdaqDownloader(BaseDownloader):
            ...
            for row in reader:
                try:
                    ...
                    close = self._parse_price(
                        row.get("Close/Last", row.get("Close", row.get("close", "0")))
                    )
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    ...
                    ex_date = self._parse_date(
                        row.get("Ex/EFF DATE", row.get("Ex-Date", row.get("ex_date", "")))
                    )
                    payment_date = self._parse_date(
                        row.get(
                            "PAYMENT DATE", row.get("Payment-Date", row.get("payment_date", ""))
                        )
                    )
                    amount = self._parse_price(
                        row.get("CASH AMOUNT", row.get("Amount", row.get("amount", "0")))
                    )
    ...
    def _parse_company_info(self, filepath: Path) -> list[StockDocument]:
        ...
        try:
            ...
            if isinstance(data, list):
                docs = [self._json_to_document(item) for item in data if item]
    ...
    def _json_to_document(self, data: dict[str, Any]) -> StockDocument | None:
