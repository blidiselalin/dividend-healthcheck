...
class NasdaqFetcher(BaseFetcher):
    ...
    def __init__(self, output_dir: str = "data/downloads/nasdaq"):
        """
        Initialize fetcher.
    ...
    def download_multiple(
        ...
        for symbol in symbols:
            try:
                logger.info(f"Processing {symbol} ({stats['symbols_processed'] + 1}/{len(symbols)})")
    ...
    def _save_prices_csv(self, symbol: str, prices: List[Dict]) -> None:
    ...
    def _save_dividends_csv(self, symbol: str, dividends: List[Dict]) -> None:
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["symbol", "ex_date", "payment_date", "record_date", "amount", "type"],
                extrasaction="ignore",
            )
            writer.writeheader()
    ...
    def _save_info_json(self, info_list: List[Dict]) -> None:
