#!/usr/bin/env bash
# Refresh live prices in the shared market library every 30 minutes (cron-friendly).
#
# Usage:
#   cd ~/dividend-healthcheck && ./scripts/price_refresh_30min.sh
#
# Cron (install via ./scripts/install_price_refresh_cron.sh):
#   */30 * * * * cd ~/dividend-healthcheck && ./scripts/price_refresh_30min.sh
set -euo pipefail
...
{
  echo ""
  echo "=== $(date -Is) price refresh (30min) ==="
